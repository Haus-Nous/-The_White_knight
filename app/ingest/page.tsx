...
// Minimal mock target buckets for demonstration without full profile load
const MOCK_BUCKETS: TargetBucket[] = [
  {
    id: "ai-product",
    name: "AI Product",
    description: "Senior AI/ML product roles",
    titlesMatch: ["ai product manager", "agentic", "ml product"],
    titlesExclude: ["junior"],
    sectorsPreferred: ["saas", "fintech", "ai-frontier-tech"],
    geographies: ["India", "UAE", "Remote"],
    keywordsRequired: ["ai", "ml", "llm", "agentic"],
...
export default function IngestPage() {
  ...
  const handleScore = () => {
    setIsScoring(true);
    setTimeout(() => {
      const result = scoreJob(jdText, company, role, location, seniority, sector, MOCK_BUCKETS);
      setScoreResult(result);
      setIsScoring(false);
    }, 600); // fake delay for feel
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <main className="container" style={{ paddingTop: 24, paddingBottom: 64, flex: 1, maxWidth: 800 }}>
        ...
        <div style={{ background: "var(--bg-secondary)", padding: 24, borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <button className="btn btn-primary" onClick={handleScore} disabled={!company || !role || !jdText || isScoring} style={{ width: "100%", padding: 12, justifyContent: "center" }}>
              {isScoring ? "SCORING..." : "SCORE JOB AGAINST PERSONA"}
            </button>
          </div>

          {scoreResult && (
            <div style={{ marginTop: 24, borderTop: "1px solid var(--border)", paddingTop: 24 }}>
              <div className="section-title" style={{ marginBottom: 16 }}>SCORE RESULT</div>
              
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg-primary)", padding: 16, border: "1px solid var(--border)" }}>
                <div>
